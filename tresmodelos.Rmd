...
Esta es una lectura digerida de un fragmento del capítulo 2 de [_The Elements of Statistical Learning_](http://statweb.stanford.edu/~tibs/ElemStatLearn/) de Hastie, Tibshirani y Friedman. Había algunos detalles que no tenía claros y reproducir lo que hacen me ayuda a entender. 
...
En los siguientes dos apartados estudiaremos dos formas básicas para establecer procedimientos de clasificación.
...
plot_errores <- ggplot(errors, aes(x=degrees, y=value, col=variable)) + 
  ...
  scale_color_discrete(name="Muestra", labels=c("Entrenamiento", "Evaluación")) +
  geom_point(data=data.frame(x=lm.df, y=error_training_linear), aes(x=x,y=y), color="firebrick1", shape= 15, size=5) +
  geom_point(data=data.frame(x=lm.df, y=error_test_linear), aes(x=x,y=y), color="chartreuse3", shape= 15, size=5) + 
...
Las curvas azul aguamarino y roja clara son versiones suavizadas de las curvas de error (atrás difuminadas) de los modelos de vecinos cercanos para $k=2n-1$ con $n=1,2, \ldots, 100$ pero en lugar de $k$ uso los grados de libertad de cada modelo (en este caso 200/k) y lo escalo logarítmicamente. 
...
Para calcular el valor exacto de este error tendría que integrar funciones de densidad sobre las dos áreas que demarca el clasificador. Como no tengo ni idea de cómo hacer eso, un método alternativo (no sé qué tan acertado) podría ser calcular el promedio de los errores para una serie grande de muestras. Como eso tarda, pregeneré una muestra de 100.000 puntos coloreados y para cada uno calculé su clasificación según Bayes: 
...
ggplot(super_sample, aes(x=1:nrow(super_sample), y=cumerror)) + geom_line(color="goldenrod2") + theme_bw() + xlab("Tamaño de muestra") + ylab("Error")
```

El error promedio de esta pila de 100.000 muestras es `r mean(super_sample$cumerror)`. Más o menos ese es el valor del error óptimo.

Por comparar hagamos el mismo procedimiento usando el clasificador de los $35$-vecinos cercanos y miremos cómo se comporta el error a medida que la muestra crece:
...
ggplot(ssamp, aes(x=index, y=value, col=variable)) + geom_line() + theme_bw() + xlab("Tamaño de muestra") + ylab("Error") + scale_color_discrete(name="Modelo", labels=c("35-vecinos cercanos", "Clasificador de Bayes")) + theme(legend.position="bottom")
```
...
```{r,  fig.height=7, fig.width=8, fig.align='center'}
plot_errores + geom_abline(slope=0, intercept=mean(super_sample$cumerror), color="red", linetype="dashed", alpha=0.6) + annotate("text", x=100, y=mean(super_sample$cumerror)+0.006, label="Error óptimo de Bayes (estimado)", size=4)
```
